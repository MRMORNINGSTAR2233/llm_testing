...
class AdvancedChainOfThoughtModel(nn.Module):
    def __init__(self, base_model_name: str, num_reasoning_steps: int = 1):
        super().__init__()
        ...
        hidden_size = self.base_model.config.hidden_size // 2
        self.reasoning_modules = nn.ModuleList([nn.Linear(hidden_size, hidden_size) for _ in range(num_reasoning_steps)])
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        hidden_states = [base_outputs.hidden_states[-1][:, :, :self.base_model.config.hidden_size//2]]
